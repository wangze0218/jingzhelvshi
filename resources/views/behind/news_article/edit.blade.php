        ...
        <div class="col-md-12">
            <section class="panel">
                <div class="panel-body">
                    <form action="/behind/news_article/{{$article->article_id}}" role="form" id="from" method = 'post' onsubmit="return update('/behind/news_article/{{$article->article_id}}');" enctype="multipart/form-data" class="form-horizontal tasi-form">
                            ...
                            <div class="col-md-4">
                                <select class="colorpicker-default form-control" name="rel_type_id" id="">
                                    @foreach($article_all_type as $k=>$v)
                                        <option @if($article->rel_type_id == $v['news_id']) selected @endif value="{{$v['news_id']}}">{{$v['news_name']}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
